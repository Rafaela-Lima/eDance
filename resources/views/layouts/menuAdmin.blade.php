            ...
            <ul style="padding-left: 0px">
                <li>
                    <form id="logout-form" action="{{ route('user.logout') }}" method="POST">
                        @csrf
                        <button style="" class="btn btn-primary" type="submit">SAIR</button>
                    </form>
                </li>
            </ul>
...
<div class="container">
    <div class="table-responsive">
        <table class="table" border=100 width=300 height=300 align=center>
            <thead>
            <?php
            $turmas = DB::table('turmas')->where('deleted_at', '=', NULL)->count();
            $alunos = DB::table('alunos')->where('deleted_at', '=', NULL)->count();
            $matriculas = DB::table('matriculas')->where('deleted_at', '=', NULL)->count();
            $funcionarios = DB::table('funcionarios')->where('deleted_at', '=', NULL)->count();
            $estudios = DB::table('estudios')->where('deleted_at', '=', NULL)->count();
            $modalidades = DB::table('modalidades')->where('deleted_at', '=', NULL)->count();


            $tabela = array
            (
                array("Controle Operacional", "Quantidades"),
                array("Turmas", "$turmas"),
                array("Alunos", "$alunos"),
                array("Funcionários", "$funcionarios"),
                array("Matrículas", "$matriculas"),
                array("Modalidades", "$modalidades"),
                array("Estúdios", "$estudios"),
            );
